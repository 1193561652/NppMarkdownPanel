...
#include <cstring>
namespace {
const NppPluginHostInfo* host=nullptr;QString iniPath,pluginHome,cssFile,htmlFile;int zoom=130;bool showToolbar=false,syncCaret=false;QPointer<QDockWidget> dock;QPointer<QTextBrowser> preview;QPointer<QToolBar> toolbar;QPointer<QTimer> renderTimer;
...
QString readCss(const QString& path){QFile f(path);return f.open(QIODevice::ReadOnly)?QString::fromUtf8(f.readAll()):QString();}
QString css(){if(!cssFile.isEmpty()&&QFileInfo::exists(cssFile))return readCss(cssFile);const QStringList defaults={QDir(pluginHome).filePath(QStringLiteral("NppMarkdownPanel/style.css")),QDir(pluginHome).filePath(QStringLiteral("style.css")),QDir(QApplication::applicationDirPath()).filePath(QStringLiteral("plugins/NppMarkdownPanel/style.css")),QStringLiteral(":/NppMarkdownPanel/style.css")};for(const QString&candidate:defaults){const QString value=readCss(candidate);if(!value.isEmpty())return value;}return QString();}
...
void render(){if(!preview)return;const QString directory=QFileInfo(currentPath()).absolutePath();preview->setSearchPaths({directory});preview->document()->setBaseUrl(QUrl::fromLocalFile(QDir(directory).absolutePath()+QLatin1Char('/')));const QString source=QString::fromUtf8(text());const QString style=css();preview->setHtml(MarkdownCore::toHtml(source,style));QFont font=QApplication::font();font.setPointSizeF(font.pointSizeF()*zoom/100.0);preview->setFont(font);if(toolbar)toolbar->setVisible(showToolbar);if(!htmlFile.isEmpty()){QFile f(htmlFile);if(f.open(QIODevice::WriteOnly))f.write(MarkdownCore::toHtml(source,style).toUtf8());}}
